...
def delete_ambassador(event):
    ...
    try:
        # First get the ambassador to delete associated S3 files
        result = ambassadors_table.get_item(Key={'id': ambassador_id})
        ambassador = result.get('Item')
        
        if ambassador:
            # Delete S3 files (optional - might want to keep for audit)
            # delete_ambassador_media(ambassador)
            pass
...
def get_upload_url(event):
    ...
    folder = body.get('folder', 'uploads')  # profiles, photos, videos
...
def get_public_ambassadors(event):
    ...
    try:
        # Only get ambassadors that are recommended or have been chosen
...
def lambda_handler(event, context):
    ...
    routes = {
        ...
        # Ambassadors routes
        ('GET', '/api/ambassadors'): get_public_ambassadors,  # Public
